...
with ctePreDrugTarget as (
  ...
  from @dest_schema.drug_exposure as d
  inner join
    @dest_schema.concept_ancestor as ca
    on d.drug_concept_id = ca.descendant_concept_id
  inner join @dest_schema.concept as c on ca.ancestor_concept_id = c.concept_id
  where
    ...
    (
      ...
      from (
        ...
        select
          person_id,
          ingredient_concept_id,
          date_add(drug_sub_exposure_end_date, 30) as event_date,
          1 as event_type,
          NULL
        from cteFinalTarget
      ) as RAWDATA
